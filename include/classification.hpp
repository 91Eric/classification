...
class Classifier {  
 public:  
  ...
  std::vector<Prediction> Classify(const cv::Mat& img, int N = 5);//Classify函数去进行网络前传，得到img属于各个类的概率的TOPN N 的概率和类别  
  
 private:  
  void SetMean(const string& mean_file);//SetMean函数主要进行均值设定，每张检测图输入后会进行减去均值的操作，这个均值可以是模型使用的数据集图像的均值  
  
  std::vector<float> Predict(const cv::Mat& img);//Predict函数是Classify函数的主要组成部分，将img送入网络进行前向传播，得到img属于各个类的概率    
